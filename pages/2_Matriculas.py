...
def _render_matrices(base,roster,roster_por_mes,is_business_day,global_supervisor):
    """`roster` es el universo COMPLETO de supervisores/asesores (sin recorte de
    fecha ni de mes) — de ahí sale la lista base de nombres posibles. Cada matriz
    se filtra además por `roster_por_mes` (ver `_datos.roster_matriculas_por_mes`)
    usando el mes que ESE panel está mostrando: alguien activo en enero pero ya
    retirado en septiembre sigue saliendo en la matriz de enero, no en la de
    septiembre. `base` (ya filtrada por fecha/mes) sigue siendo la fuente de los
    valores dia a dia."""
    months=_available_months(base)
...
def _css():
    ...
    div.st-key-mat_module_nav{margin:0 0 8px;padding:5px;border:1px solid rgba(255,255,255,.08);border-radius:14px;background:rgba(255,255,255,.03)}div.st-key-mat_module_nav div[data-testid='stHorizontalBlock']{gap:6px}div.st-key-mat_module_nav button{min-height:40px!important;border:1px solid transparent!important;border-radius:10px!important;background:transparent!important;color:rgba(255,255,255,.55)!important;box-shadow:none!important;font-size:11px!important;font-weight:650!important;transition:background .16s,color .16s,border-color .16s!important}div.st-key-mat_module_nav button:hover{color:white!important;background:rgba(255,255,255,.045)!important;border-color:rgba(255,255,255,.08)!important}div.st-key-mat_module_nav button[kind='primary']{color:#7DD3FC!important;background:color-mix(in srgb,#38BDF8 14%,transparent)!important;border-color:color-mix(in srgb,#38BDF8 30%,transparent)!important;box-shadow:none!important}
